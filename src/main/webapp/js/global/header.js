...
$(document).ready(function () {
    ...
    var info = getAuthenticationInformation();
    console.log(info.isKeycloak);
    console.log(info.isKeycloak === true);
    sessionStorage.setItem("isKeycloak", info.isKeycloak);
    if (info.isKeycloak === true) {
        console.log("Connecting KeyCloak");
        keycloak = connectKeycloak(info);
...
//            }
    $.ajax({url: "./information/authenticationMode",
        async: false,
        dataType: 'json',
        //headers: headers,
 ...
 * @returns {undefined}
 */
function connectKeycloak(data) {
    var keycloak = new Keycloak({
        url: data.keycloakUrl,
        realm: data.keycloakRealm,
        clientId: data.keycloakClient
    });
    keycloak.init({
